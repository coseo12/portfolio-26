...
```
                          [Hero] [About Me] [Projects] [Contact(우측)]
```
...
| ID | 각 섹션에 `id` 부여 (`hero`, `about`, `projects`, `contact`) |
...
├── views/
│   └── home/
│       └── index.tsx             ← 섹션 조합 (Hero + About + Projects + Contact)
│
├── widgets/
│   └── header/
...
| 모바일 (`md` 미만) | 헤더 메뉴 가로 나열 유지 (항목 4개로 적음) |
...
- [ ] 4개 섹션(Hero, About, Projects, Contact)이 순서대로 배치된다
- [ ] 각 섹션이 그라디언트 페이드로 자연스럽게 연결된다
